$(function () {
    function a() {
        ...
        for (var j = 0; j < g.length; j++) {
            ...
            k.onload = function () {
                h++;
                if (h == g.length) {
                    $(".frontpage").children().removeClass("hide");
                    d.init();
                    setTimeout(function () {
                        window.scrollTo(0, 0)
                    }, 500)
    ...
    function b() {
        $("#j_reload").on("click tap", function () {
            document.location.reload()
        });

        if (f()) {
            $("#j_weixin").removeClass("hide");
            e()
        }
    ...
    function f() {
        ...
        if (g.match(/MicroMessenger/i) == "micromessenger") {
            return true
        }
    ...
    var d = {
        config: examConfig,
        frontPage: $(".frontpage").eq(0),
        enter: $(".j_enter").eq(0),
        _endPage: $(".endpage").eq(0),
        sumDesc: $("#j_desc"),
        questionList: c(examConfig.questionList),
        len: examConfig.questionList.length,        section: $(".frontpage").eq(0),
        pages: null,
        answers: null,
        sum: 0,
        level: 1,
        comments: examConfig.comments,
        names: examConfig.names,
        timer: null,
        pageRender: function (g) {
            var h = d.questionList[g], j = "";
            j += '<section class="page" data-turn="' + (g + 1) + '"><h3 style="font-size:20px;">第<span style="color:black; font-size:24;">' + (g + 1) + "</span>题</h3>";
            j += '<div class="title"><div class="image"><img src="' + h.image + '"></div><p>' + h.question + "</p></div>";
            j += '<ul class="choices narrow">';
            if (h.answer == "A") {
                j += '<li><span class="j_answer" data-answer="1">' + h.answerA + "</span></li>"
            }
            ...
            j += "</ul></section>";
            $(j).insertBefore(d.section);
            d.section = $(".page").first();
            d.pages = $(".page");
            d.answers = $(".j_answer");
            d.choiceInit()
        },
        choiceInit: function () {
            d.answers.on("click tap", function () {
                var k = $(this), i = k.parent(), l = k.parents(".page").eq(0), h = l.prev().prev() || null, g = parseInt(l.data("turn"));
                if (l.data("clicked") == "yes") {
                    return
                }
                    ...
                    l.data("clicked", "yes");
                    i.addClass("down");
                    if (k.data("answer") && k.data("answer") == 1) {
                        i.addClass("correct");
                        d.sum++
                    }
                ...
                if (h && h.find("img").length > 0) {
                    var j = h.find("img").eq(0);
                    j.attr("src", j.data("src"))
                }
            })
        },
        resultPage: function () {
            d.level = Math.floor((d.sum - 1) / 5);
        ...
        enterInit: function () {
            d.enter.on("click tap", function () {
                var g = $(this);
                g.closest("section").addClass("fly")
            })
        },
        init: function () {
            for (var g = 0; g < 2; g++) {
